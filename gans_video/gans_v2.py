"""
GANs, rebuilt.  One continuous Scene - see SCRIPT.md for the narration.

Deliberately a single Scene rather than eleven. The v1 video rendered each
beat as its own file and concatenated them, which is why 10 of its 11 scene
boundaries passed through a blank black frame. Objects here persist across
...
# sys.path, so a sibling helper module is not importable without this.
...
from gan_lib import (
    ...
    audit_text_overlaps, Landscape, FONT,
)
...
class GANs(Scene):
    ...
    def swap_field(self, real, fake, tag, run_time=1.5, extra=()):
        ...
        is genuinely Transformed at the same time, and that is what carries
        object identity through the change.
        """
        new = self.land.field_image(real, fake, tag, self.cache)
        new.set_opacity(0.0)
        self.add(new)
        self._front()
        self.play(
    ...
    def s3_landscape(self):
        ...
        self.land = land
        self.cache = str(Path(__file__).parent / "_field_cache")

        field = land.field_image(REAL_SPEC, FAKE_START, "start", self.cache)
