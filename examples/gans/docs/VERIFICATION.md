# Phase 4 — Verification of `videos/GANs.mp4`

**Render:** 375.0s (6:15), 1920×1080, 30fps, single continuous `Scene`.
**Command:** `bash ../../skills/3b1b-math-animation/scripts/render.sh gans.py GANs`
**Checks:** `python ../../skills/3b1b-math-animation/scripts/verify_render.py videos/GANs.mp4 --meta render_meta.json`
...
Camera push-ins are declared by the scene (`manim_helpers.push_in` → `render_meta.json`)
so checks [1] and [4] stay strict everywhere else rather than being loosened
globally to accommodate two deliberate close-ups.

## Text-overlap audit

`GANS_AUDIT=1 manimgl gans.py GANs` walks every visible `Text`/`Tex` on
...
|---|---|---|---|
| **A1** | Every `Dot` renders white; `Arrow`/`Circle` too | **fixed** | Colour check: 64% of lit pixels saturated vs v1's ~4%. All dots go through `manim_helpers.dot()`, which sets colour after construction. Root cause proven empirically: `Dot(color=ORANGE)`→RGB(253,255,253); `Dot().set_color(ORANGE)`→RGB(255,123,0) |
| **A2** | Colour mapping inconsistent (BLUE = real / generated / loss; YELLOW 4 ways) | **fixed** | One mapping declared in `gans.py` and in SCRIPT.md; blue is real-only, orange generated-only, teal discriminator-only. Yellow is transient emphasis and is no longer the generator's identity colour |
...
| **D1** | Highlight box indexed by source-string length → landed on nothing; second box wrapped the whole equation | **fixed** | Equation built from separately addressable `Tex` terms via `manim_helpers.equation()`; each term is a real mobject, boxed with `box_around(term)`. No glyph-array indexing anywhere |
...
| **D4** | Dead code (`boundary()` never called) | **fixed** | No unused functions in `gans.py` / `landscape.py` |
...
Four v1 defects (B4, C2, C7, D3) are marked **n/a**: the visuals that carried them
